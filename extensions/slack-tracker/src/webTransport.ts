...
export class SlackWebTransport implements SlackTransport {
  ...
  async readFile(fileId: string, abortSignal?: AbortSignal): Promise<SlackFileDownload> {
    const info = await this.get("files.info", { file: fileId }, abortSignal);
    ...
    if (!response.body) throw new Error(`slack file download returned no body: ${fileId}`);
  ...
  async postReply(
    ...
    if (options.files?.length) {
      ...
      for (const file of options.files) {
        await file.withBody(async (body, size) => {
          const prepared = await this.post(
            "files.getUploadURLExternal",
            { filename: file.filename, length: size },
            { idempotent: false },
          );
          ...
          uploaded.push({ id: prepared.file_id });
        });
      }
      await this.post(
...
function toMessage(channel: string, m: RawSlackMessage, botUserId?: string): SlackMessage {
  ...
  const files = slackFilesFromRaw(m.files);
  return {
    channel,
    ts: m.ts ?? "",
    text: m.text ?? "",
...
/** Parse Slack file objects without exposing private download URLs. */
export function slackFilesFromRaw(raw: unknown): SlackFile[] {
