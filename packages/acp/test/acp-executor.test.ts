...
test("ACP executor starts a session, translates updates, approves permissions, and exposes fs", async () => {
  ...
  assert.match(JSON.stringify(newSession.params), /"name":"lorenz_jira"/);
  assert.match(
...
test("ACP executor acquires AND releases its OWN endpoint when no mcpEndpoint is threaded", async () => {
  ...
  // default lorenz_jira server) and releasing the session releases it.
  assert.match(JSON.stringify(session.mcpEndpoint.acpServer()), /"name":"lorenz_jira"/);
  await session.stop();
