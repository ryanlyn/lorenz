...
export interface SerializedOrchestratorState {
  ...
  exhausted?: Array<[string, ExhaustedEntry]> | undefined;
  completed: string[];
...
export function serializeState(state: OrchestratorState): SerializedOrchestratorState {
  return {
    ...
    exhausted: [...state.exhausted.entries()].map(([key, entry]) => [key, { ...entry }]),
...
export function hydrateState(
  ...
  state.exhausted = new Map((snapshot.exhausted ?? []).map(([key, entry]) => [key, { ...entry }]));
