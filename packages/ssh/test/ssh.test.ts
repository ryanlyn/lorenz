...
test("SSH reverse tunnel waits for delayed stderr and retains a bounded tail", async () => {
...
`,
  ...
  assert.equal(process.readStderrTail(), "");
  ...
  const stderrTail = process.readStderrTail();
