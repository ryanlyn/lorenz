...
test("runtime workflow reloader applies changed workflow transactionally", async () => {
  ...
  const orchestrator: Pick<Orchestrator, "settings"> = { settings: workflow.settings };
...
test("runtime dispatcher refreshes, claims, records dispatch, and tracks run lifecycle", async () => {
  ...
  const orchestrator: Pick<Orchestrator, "claimAsync" | "abandonClaimAsync"> = {
...
test("runtime reconciler cleans terminal tracked issues and clears local runtime state", async () => {
  ...
  const orchestrator: Pick<
    ...
    | "finishAsync"
  > = {
    trackedIssueSnapshotAsync: async () => ({
      claims: [
        {
    ...
    ownedRunningEntriesAsync: async () => [],
    runningEntryAsync: async () => undefined,
    finishAsync: async () => null,
...
test("runtime snapshot projector preserves facade snapshot shape", () => {
  ...
  assert.equal(snapshot.appStartedAt, "2026-01-01T00:00:00.000Z");
  assert.equal(snapshot.running[0]?.runId, "run-1");
  assert.equal(snapshot.running[0]?.issueIdentifier, "MONO-1");
  assert.notEqual(snapshot.running[0]?.usageTotals, running.usageTotals);
  assert.equal(snapshot.reserving?.[0]?.slotIndex, 1);
  assert.notEqual(snapshot.reserving?.[0], orchestration.reserving[0]);
