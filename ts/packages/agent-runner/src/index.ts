...
class RunController {
  ...
  async run(): Promise<RunResult> {
    ...
    input.onUpdate?.({ type: "workspace_prepared", workspacePath: workspace, message: `workspace prepared at ${workspace}` });
    ...
    try {
      while (turnCount < runtime.agent.maxTurns) {
        ...
        if (
          ...
          !turnUpdates.some((u) => u.type === "session_notification" && u.message?.update?.sessionUpdate === "tool_call")
        ) {
          break;
        }
