...
test("INVARIANT: When the global concurrency cap is reached, the system SHALL NOT dispatch new work - eligibleIssues blocks dispatch when running map size equals global maxConcurrentAgents", () => {
...
test("INVARIANT: When a per-state concurrency cap is reached, the system SHALL block only issues in that state - eligibleIssues applies per-state cap only to running entries in that state", () => {
...
test("INVARIANT: When dispatch is evaluated, both global and per-state caps SHALL have room simultaneously - dispatch requires both global and state caps to have room simultaneously", () => {
  const settings = parseConfig({
...
test("INVARIANT: When all worker hosts are at capacity, the system SHALL NOT dispatch new work - worker host running count derived from running map entries per host", () => {
...
test("INVARIANT: When multiple hosts are tied at the lowest load, the system SHALL select the first in config order - host selection picks first host in config order on equal load tie", () => {
  const settings = defaultSettings();
  settings.agent.maxConcurrentAgents = 10;
...
test("INVARIANT: When a slot is already claimed, a repeated claim for the same issue SHALL return null - claimed slot cannot be double-claimed by repeated claim calls", () => {
...
test("INVARIANT: When a claim succeeds, the retryAttempts entry for that issue SHALL be deleted - successful claim deletes the retryAttempts entry for the issue", () => {
  const clock = makeClock(1000000);
...
test("INVARIANT: When an entry is finished, its slot key SHALL be removed from both the running map and claimed set - finish removes slot key from running map and claimed set", () => {
  const clock = makeClock(1000000);
  const settings = makeSettings({ maxConcurrent: 10 });
  const orch = new Orchestrator(settings, clock);
  const issue = makeIssue();
...
test("INVARIANT: When all ensemble slots are claimed, the system SHALL return null on subsequent claim attempts - issue with all ensemble slots claimed returns null on subsequent claim", () => {
  fc.assert(
...
test("INVARIANT: When a retry becomes due, stale claimed slots SHALL be released and re-dispatch SHALL proceed - stale claim released when retry becomes due and re-dispatch proceeds", () => {
  const clock = makeClock(1000000);
  const settings = makeSettings({ maxConcurrent: 10 });
  const orch = new Orchestrator(settings, clock);
  const issue = makeIssue();
...
test("INVARIANT: When an update targets a nonexistent slot key, the system SHALL NOT throw or mutate state - applyUpdate with nonexistent slotKey does not throw and does not mutate state", () => {
  fc.assert(
    fc.property(
...
test("INVARIANT: When a turn_completed event is applied, turnCount SHALL increment by exactly one - turnCount increments by exactly one per turn_completed event", () => {
  fc.assert(
...
test("INVARIANT: When a usage update reports lower values, entry usageTotals SHALL NOT decrease - entry usageTotals never decrease even when update reports lower values", () => {
  const clock = makeClock(1000000);
  const settings = makeSettings({ maxConcurrent: 10 });
  const orch = new Orchestrator(settings, clock);
  const issue = makeIssue();
...
test("INVARIANT: When global usage totals are updated, growth SHALL be bounded by positive deltas from highwater marks - global usageTotals only accumulates positive deltas from lastReported highwater marks", () => {
  const clock = makeClock(1000000);
  const settings = makeSettings({ maxConcurrent: 10 });
  const orch = new Orchestrator(settings, clock);
  const issue = makeIssue();
...
test("INVARIANT: When a running issue is refreshed, the update SHALL propagate to every ensemble slot for that issue - refreshRunningIssue updates issue object on every ensemble slot for the same issue", () => {
  fc.assert(
...
test("INVARIANT: When an issue is cleaned up, all running entries, claimed slots, and retry state SHALL be removed - cleanupIssue removes all running entries, claimed slots, and retry for the issue", () => {
  const clock = makeClock(1000000);
...
test("INVARIANT: When an entry is finished, elapsed seconds SHALL be computed from startedAt to clock.now and accumulated - finish computes elapsed seconds from startedAt to clock.now and accumulates", () => {
...
test("INVARIANT: When a continuation finish occurs, the issue SHALL be added to both the completed set AND retryAttempts (scheduling re-dispatch) - continuation finish adds issue to completed set and creates retry entry for re-dispatch", () => {
  const clock = makeClock(1000000);
  const settings = makeSettings({ maxConcurrent: 10 });
  const orch = new Orchestrator(settings, clock);
  const issue = makeIssue();
...
test("INVARIANT: When a snapshot is taken, its arrays and objects SHALL be independent copies that do not alias internals - snapshot arrays and objects are independent copies that do not alias orchestrator internals", () => {
  const clock = makeClock(1000000);
  const settings = makeSettings({ maxConcurrent: 10 });
  const orch = new Orchestrator(settings, clock);
  const issue = makeIssue();
