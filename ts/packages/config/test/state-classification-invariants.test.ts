...
test("INVARIANT: When states are normalized, normalization SHALL be case-insensitive. - normalizeStateName — mixed case variants all normalize to the same value", () => {
  fc.assert(
    fc.property(
...
test("INVARIANT: When state normalization is applied twice, the result SHALL be the same. - normalizeStateName — applying normalization twice yields the same result as once (idempotent)", () => {
  fc.assert(
...
test("INVARIANT: When states are normalized, leading and trailing whitespace SHALL be stripped. - normalizeStateName — leading and trailing whitespace is stripped", () => {
...
test("INVARIANT: When a state value is null or undefined, it SHALL be classified as non-terminal. - isTerminalState — null and undefined states are always classified as non-terminal", () => {
...
test("INVARIANT: When a state value is unknown, it SHALL be classified as non-terminal. - isTerminalState — a state not in the terminal list is classified as non-terminal", () => {
  fc.assert(
    fc.property(
      fc.string({ minLength: 1, maxLength: 30 }),
...
test("INVARIANT: When states are compared, comparison SHALL be case-insensitive and whitespace-tolerant. - isTerminalState — comparison is case-insensitive (any casing of a terminal state matches)", () => {
  fc.assert(
    fc.property(
...
test("INVARIANT: When two states normalize to the same value, they SHALL produce the same terminal classification. - isTerminalState — a state matches its own normalized form in the terminal list", () => {
  fc.assert(
    fc.property(
