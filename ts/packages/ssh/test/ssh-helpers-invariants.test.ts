...
test("INVARIANT: When shellEscape is applied, the output SHALL be reversible to the original input by a POSIX shell - shellEscape wraps value in single quotes preventing unquoted shell metacharacters", () => {
  fc.assert(
...
test("INVARIANT: When an SSH target is parsed, destination and port SHALL be correctly separated - parseSshTarget roundtrip - destination:port recombines correctly", () => {
  fc.assert(
...
test("INVARIANT: When sshArgs constructs arguments, they SHALL be consistent with parseSshTarget output - sshArgs uses parseSshTarget destination as the host argument", () => {
  fc.assert(
    fc.property(sshTargetAny, shellDangerousString, (host, command) => {
      const args = sshArgs(host, command);
