...
test("INVARIANT: When a workspace path is resolved, it SHALL be a strict descendant of the workspace root. - workspace path is a strict descendant of the workspace root", () => {
  fc.assert(
...
test("INVARIANT: When directory names are derived from identifiers, they SHALL contain only alphanumeric characters, dots, hyphens, and underscores - safeIdentifier output contains only alphanumeric, dots, hyphens, underscores", () => {
  fc.assert(
...
test("INVARIANT: When sanitization is applied to a name, applying it again SHALL produce the same result. - safeIdentifier is idempotent on arbitrary strings", () => {
  fc.assert(
...
test("INVARIANT: When a multi-slot ensemble is resolved, each slot SHALL receive a distinct workspace path. - ensemble slots produce distinct workspace paths", () => {
  fc.assert(
    fc.property(
      absoluteRoot,
      validIdentifier,
...
test("INVARIANT: When a single-slot run is resolved, the workspace path SHALL have no slot suffix. - single-slot run has no slot suffix in path", () => {
  fc.assert(
    fc.property(absoluteRoot, validIdentifier, (root, identifier) => {
...
test("INVARIANT: When a workspace path is produced, it SHALL be a valid absolute path. - additional: workspacePath always produces an absolute path", () => {
  fc.assert(
    fc.property(
      absoluteRoot,
      validIdentifier,
...
test('INVARIANT: When a workspace path is produced, it SHALL contain no ".." segments - additional: workspacePath output never contains parent directory traversals', () => {
  fc.assert(
    fc.property(
      absoluteRoot,
