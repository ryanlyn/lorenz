...
/**
 * Configuration for the embedded warm worker/executor box pool. When present and `enabled`,
 * the runtime leases an SSH-addressable box per run (producing the run's `workerHost`) instead
...
export interface BoxPoolSettings {
  ...
  max: number;
  /** Target number of pre-warmed idle boxes the reaper tops up toward, within budget. */
  warm: number;
  /**
   ...
   *
   * This is the single canonical own field. {@link BoxPoolSettings.maxInFlight} is a derived,
  ...
  /**
   * @deprecated Read-only alias of {@link BoxPoolSettings.slotsPerMachine}. Declared as a get
  ...
  /** Hard lifetime (ms) of a box before it is recycled, even while idle. */
  ttlMs: number;
  /** Idle window (ms) after which a warm box above `min` is reaped. */
  idleReapMs: number;
  ...
  /** Deadline (ms) `drain` waits for in-flight leases before force-destroying all boxes on shutdown. */
  drainDeadlineMs: number;
  /** Optional fairness cap on how many boxes one issue's slots may hold, so an ensemble cannot monopolize the pool. */
  maxBoxesPerIssue?: number | undefined;
  /**
   * Explicit operator opt-in for co-residence (multiple run slots sharing one machine). Required
   * alongside the runtime per-run-endpoint capability before {@link BoxPoolSettings.slotsPerMachine}
   * may exceed `1`: a single poisoned box fails every co-resident run on recycle, so widening the
  ...
  /** Optional spend caps. Box counts are absolute; box-seconds are wall-clock seconds. */
  spend?:
    | {
        /** Maximum boxes that may exist concurrently (independent of `max`). */
        maxConcurrentBoxes?: number;
        /** Maximum cumulative box-seconds for the process lifetime. */
        maxBoxSeconds?: number;
        /** Maximum box-seconds per UTC day; persisted across restart via a `spend.json` sidecar. */
        dailyBoxSeconds?: number;
      }
    | undefined;
  /**
...
/**
 * Shape callers construct: all of {@link BoxPoolSettings} except the derived `maxInFlight`
 ...
 */
export type BoxPoolSettingsInput = Omit<BoxPoolSettings, "maxInFlight">;

/**
 * Installs the derived, read-only `maxInFlight` getter over `slotsPerMachine` so the two can
 * never drift. This is the single sanctioned constructor for {@link BoxPoolSettings}: the input
 ...
 * structurally a plain `BoxPoolSettings` for callers that spread or snapshot it.
 *
 * Clone interaction (documented per task note): the accessor is defined `enumerable: true`, so a
 * shallow spread (`{ ...boxPool }`, as used by `cloneSettings`/`cloneBoxPool`) or
 ...
 */
export function withDerivedMaxInFlight(input: BoxPoolSettingsInput): BoxPoolSettings {
  return Object.defineProperty(input, "maxInFlight", {
    get(this: BoxPoolSettingsInput): number {
...
export interface WorkerSettings {
  ...
  /**
   * Optional embedded box-pool configuration. Absent (the default) leaves the static-host path
   * unchanged. When `enabled`, the pool leases SSH-addressable boxes to produce each run's
   * `workerHost`; it cannot be combined with a non-empty {@link WorkerSettings.sshHosts}.
   */
  boxPool?: BoxPoolSettings | undefined;
}
