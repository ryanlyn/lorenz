...
afterEach(async () => {
  ...
  activeHarness = null;
  if (!harness) return;
  // Always drain so no warm box leaks between tests, then restore process env.
...
test("memory-tracker daemon leases a fake box, completes a run, and returns it to warm", async () => {
  ...
  // The warm box's address matches the one threaded through the run history.
  assert.equal(pool.boxes[0]?.workerHost, history?.workerHost);
  assert.equal(pool.boxes[0]?.state, "WARM_IDLE");

  // The lease settled exactly as a healthy completion: the box was returned to
...
test("pool at capacity surfaces worker_host_capacity via canAcquire with no claim-then-backoff churn", async () => {
  const harness = await setupHarness({ issues: [eligibleIssue("issue-blocked", "BOX-BLOCKED")] });

  // Fill the single box (max=1, maxInFlight=1) by holding a lease, so
  ...
  const heldLease: BoxLease = held.lease;
  assert.equal(harness.pool.canAcquire(), false);

  // Real poll: the orchestrator consults boxPool.canAcquire() during
  ...
  // Release the occupant so the shared box returns to warm (afterEach drains).
  await heldLease.release("healthy");
  assert.equal(harness.pool.canAcquire(), true);
});

test("stop then drainBoxPool destroys every box (zero boxes remain)", async () => {
  const harness = await setupHarness({ issues: [eligibleIssue("issue-drain", "BOX-DRAIN")] });

  // Run once so the pool actually holds a warm box, then take the SIGINT path:
  // synchronous stop() followed by the awaited drainBoxPool() the daemon's
  ...
  await harness.runtime.drainBoxPool();

  // The leak fix: drain force-destroys all boxes so nothing survives exit.
  ...
  assert.equal(drained.boxes.length, 0);

  // drainBoxPool is idempotent (the daemon may call it more than once).
  await harness.runtime.drainBoxPool();
  assert.equal(harness.pool.snapshot().total, 0);
});

// ---------------------------------------------------------------------------
// Harness: wires the SAME objects `runDaemon` constructs (buildBoxPool +
...
async function setupHarness(
  ...
  const root = await tempDir("symphony-ts-box-e2e");
...
function eligibleIssue(id: string, identifier: string): Record<string, unknown> {
  return {
    id,
    identifier,
    title: `Warm box pool issue ${identifier}`,
    description: "Run it on a warm box.",
    state: "Todo",
