 ...
 * changes the specifier) and registers it into the box-driver registry under
 ...
 * never on the acquire path, and the `box_pool_driver_loaded` audit event
 ...
 * `box_pool_driver_module_pinned` so the pin is observable, and cache-busting
 * query strings are rejected (unbounded module-graph growth, half-initialized
 * module hazards).
 */

/** A parsed `worker.box_pool.driver` value that is not a registered kind. */
export interface BoxDriverRef {
 ...
 * EXACT registered kind always wins - {@link ensureBoxDriverLoaded} checks
 ...
 */
export function parseBoxDriverRef(driver: string): BoxDriverRef {
  ...
  if (specifier === "") {
    throw new Error(`box_pool_driver_invalid_specifier: ${driver} has an empty module specifier`);
  }
  if (exportName === "") {
    throw new Error(`box_pool_driver_invalid_specifier: ${driver} has an empty #exportName suffix`);
  }
  if (specifier.includes("?")) {
    throw new Error(
      `box_pool_driver_invalid_specifier: ${specifier} - cache-busting query strings are not ` +
...
/** Options for {@link loadBoxDriverModule}. */
export interface LoadBoxDriverModuleOptions {
...
export interface LoadedBoxDriverModule {
  module: BoxDriverModule;
  resolvedFrom: string;
}
...
/**
 * Dynamic-imports a box-driver module and validates it. Relative and absolute
 ...
 * selected named export) and passed through `assertBoxDriverModule`, so a
 * malformed module or an SDK version mismatch fails loud here - before the
 * pool, the runtime, or any provision exists.
 */
export async function loadBoxDriverModule(
  specifier: string,
  options: LoadBoxDriverModuleOptions,
): Promise<LoadedBoxDriverModule> {
  ...
  if (candidate === undefined) {
    ...
    throw new Error(
      exportName === undefined
        ? `box_pool_driver_module_invalid: ${source} has no default export` +
            (available.length > 0 ? ` (named exports: ${available.join(", ")})` : "") +
            `; export defineBoxDriver(...) as the default export or select a named export with #name`
        : `box_pool_driver_module_invalid: ${source} has no export named "${exportName}"` +
            (available.length > 0 ? ` (available: ${available.join(", ")})` : ""),
    );
  }
  assertBoxDriverModule(candidate, source);
...
function moduleResolutionError(
  ...
  if (isPathSpecifier(specifier)) {
    return new Error(
      `box_pool_driver_unavailable: ${specifier} (failed to import ${resolvedFrom}: ${message})`,
    );
  }
  const kinds =
    knownKinds.length > 0
      ? ` (known kinds: ${[...knownKinds].sort().join(", ")})`
      : " (no box drivers registered)";
  const close = closestKind(specifier, knownKinds);
  const hint = close === undefined ? "" : ` - did you mean "${close}"?`;
  return new Error(
    `box_pool_driver_unavailable: ${specifier} is not a registered kind and could not be ` +
 ...
 */
const loadedSpecifiers = new WeakMap<BoxDriverRegistry, Set<string>>();

/** Options for {@link ensureBoxDriverLoaded}. */
export interface EnsureBoxDriverLoadedOptions {
  /** Anchor for `./relative` specifiers; the daemon passes `dirname(workflow.path)`. */
...
/**
 * Idempotently makes the configured `worker.box_pool.driver` resolvable in
 ...
 * loader-registered specifier emits `box_pool_driver_module_pinned` so the
 ...
 * IS the configured specifier string, emitting `box_pool_driver_loaded`.
 *
 * Called by the daemon BEFORE `createBoxPool` at startup and (via the
 ...
 */
export async function ensureBoxDriverLoaded(
  driver: string,
  registry: BoxDriverRegistry,
  options: EnsureBoxDriverLoadedOptions = {},
): Promise<void> {
  const logEvent = options.logEvent ?? ((): void => {});
  if (registry.get(driver) !== undefined) {
    if (loadedSpecifiers.get(registry)?.has(driver)) {
      logEvent({ event: "box_pool_driver_module_pinned", specifier: driver });
    }
    return;
  }

  const ref = parseBoxDriverRef(driver);
  const { module, resolvedFrom } = await loadBoxDriverModule(ref.specifier, {
