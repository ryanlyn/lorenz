...
test("root CLI help is generated by Commander and includes subcommands", async () => {
  ...
  assert.match(output, /config \[options\] \[workflowPath\]/);
});

test("config subcommand rejects daemon-only options", async () => {
  const spy = vi.spyOn(process.stderr, "write").mockReturnValue(true);

  assert.equal(await main(["config", "--once"]), 1);

  const output = spy.mock.calls.map((call) => String(call[0])).join("");
  spy.mockRestore();

  assert.match(output, /daemon option\(s\) are not supported by lorenz config: --once/);
});
