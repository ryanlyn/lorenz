...
| Apps | `apps/cli`, `apps/lorenz-dashboard`, `apps/traceviz` | The `lorenz` / `lorenz-config` binaries, the React SPA, the standalone trace viewer |
...
| App | What it owns |
| --- | --- |
| `apps/cli` | The shipped `lorenz` and `lorenz-config` binaries and the composition root. Defines the config wizard, daemon, `runs`, and `doctor`, wires every adapter into `LorenzRuntime`, and loads out-of-tree worker drivers. First files: `src/main.ts`, `src/config.ts`, `src/daemon.ts`. |
| `apps/lorenz-dashboard` | The React/Vite SPA (package `@lorenz/dashboard`): the ops Overview and the trace viewer, behind a `#/` and `#/trace/:id` hash router. First files: `src/App.tsx`, `src/features/ops/`. |
...
The CLI shims are `apps/cli/bin/lorenz.js` and `apps/cli/bin/lorenz-config.js`, importing
`dist/bin/cli.js` and `dist/bin/config.js` respectively. See [cli.md](cli.md) and
