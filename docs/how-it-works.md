...
The agent runner builds the workspace, runs `before_run`, opens a session, then runs up to
`agent.max_turns` autonomous turns (default 20). Turn 0 sends the rendered prompt; later autonomous
turns send a continuation prompt. Human steering accepted by the active session is drained in
addition, with the same per-run limit applied separately to steering turns. Two timers guard each
turn: a hard turn timeout (`turn_timeout_ms`, default 3600000) and a stall timeout
(`stall_timeout_ms`, default 300000) reset on every agent event. Either firing rejects the active
turn and terminates its ACP session.
