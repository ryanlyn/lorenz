...
| `worker.kind` combined with `worker.ssh_hosts`, or `worker_pool.driver` with `ssh_hosts`, or `worker.kind` with `worker_pool.driver` | A driver is named twice for the same hosts. | Name the driver in one place. Also `worker_pool.max >= min` and `warm <= max` are enforced. |
