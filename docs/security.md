...
Per-run isolation comes from the Token B claim, not from a distinct network path. Co-resident runs packed onto one worker host (`worker.worker_pool.slots_per_machine > 1`) **share one** SSH reverse tunnel (`ssh -R`, one per host, refcounted by `@lorenz/worker-host-pool`); they are kept apart by their distinct claims rather than by separate ports. If the dashboard's MCP mount owns the configured port, remote runs share a separate Lorenz-owned ephemeral MCP server so their claims can still be enforced. A local run that owns no remote claim is refused rather than authorized against the settings-wide token. The token never traverses an open network port.
