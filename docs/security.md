...
- For isolated workspaces, tracker attachments are streamed by the daemon into a bounded private
  staging directory and copied to `.lorenz/attachments/`, which carries its own `*` `.gitignore`.
  The attachment protocol does not include tracker credentials or private download URLs in issue
  metadata, prompts, or MCP results. Agent processes may separately inherit daemon environment
  variables according to their agent configuration; review that policy when the daemon environment
  contains tracker credentials.
...
- **Per-run scoped claim (Token B).** An opaque token whose bytes carry no scope: it resolves server-side to a daemon-minted claim that pins the run (`runKey`, issue id, worker host, generation) and a coarse lifetime cap. The claim-enforcing MCP server re-checks the claim on **every** request - expiry, then the per-operation tool allowlist, then a liveness-and-generation re-check that returns false once the run has settled, its host was recycled, or a higher-generation re-acquire superseded it. Any miss denies; it never falls back to Token A. Verified run and issue identity is passed to tool execution; Slack's issue-scoped tools reject a different agent-supplied `issueId`. The `runKey` and authorized issue are resolved from the token, never from self-reported headers or tool arguments.
