...
| `turn_cancelled` | The bridge returned the `cancelled` stop reason. | The turn was cancelled (`acp_turn_cancelled`). Turn and stall timeouts reject the active and queued turns, terminate the session, and are followed by `process_exit`. |
