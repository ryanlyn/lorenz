...
`sessionId` is the backend session id, populated once your executor receives it. `executorPid` is the OS pid of the agent child as a string, or `null` when there is no child process. An executor that supports live user input implements `queueTurn`: it must enqueue the prompt immediately, preserve prompt order, and resolve the returned promise when the queued turn finishes. Without `options.startWhen`, submit the prompt to the backend as soon as any earlier gated prompts allow. With `options.startWhen`, keep the prompt inactive until that promise resolves. `stop` closes the session and tears down the process; it must be safe to call from a `finally` block, including before the first turn.
